...
void handleWebSocketConnection(char *message)
{
    ...
    if (event != NULL)
    {
        ...
        if (strcmp(event, "initial-state") == 0)
        {
            StaticJsonDocument<200> doc;

            String motorStatus = parseBooleanToString(motorWorking);

            doc["motorStatus"] = motorStatus;
            doc["motorDirection"] = motorClockwise ? "clockwise" : "anticlockwise";
            doc["node"] = nodeName;
            ...
            // char parsed[100];
            // String motorStatus = parseBooleanToString(motorWorking);
            // sprintf(parsed, "%s:%s", motorStatus, nodeName);
            char response[300];
            sprintf(response, "%s:%s", "initial-state-node", object);
            webSocket.sendTXT(response);
        }
